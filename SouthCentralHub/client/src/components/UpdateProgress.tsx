...
export function UpdateProgress() {
  ...
  useEffect(() => {
    const stored = getUpdateStatus();
    setTargetPercentage(stored);
  }, []);
        ...
        >
          <div className="text-center mb-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-secondary rounded-full mb-4">
              ...
              <span className="text-sm font-medium text-foreground">Status: In Development</span>
            </div>
            <div
              className="text-7xl sm:text-8xl font-bold text-primary mb-2"
              style={{
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              {percentage < 25 && "Just getting started! Big things are coming."}
              {percentage >= 25 && percentage < 50 && "Making good progress on new features."}
              {percentage >= 50 && percentage < 75 && "Over halfway there! Exciting content ahead."}
              {percentage >= 75 && percentage < 100 && "Almost ready! Final touches in progress."}
              {percentage === 100 && "Update complete! Ready for release."}
            </p>
          </div>
        </Card>
