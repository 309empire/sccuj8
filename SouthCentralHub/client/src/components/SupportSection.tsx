...
function TicketForm({ onTicketCreated }: { onTicketCreated: (ticket: Ticket) => void }) {
  const [subject, setSubject] = useState("");
  const [message, setMessage] = useState("");

  const createMutation = useMutation({
    mutationFn: (data: { subject: string; message: string }) => createTicket(data),
    onSuccess: (ticket) => {
...
function MyTickets({ onSelectTicket }: { onSelectTicket: (ticket: Ticket) => void }) {
...
export function SupportSection({ mode }: SupportSectionProps) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {mode === "discord" ? (
          ...
          <>
            ...
            <MyTickets onSelectTicket={setSelectedTicket} />
          </>
        )}
